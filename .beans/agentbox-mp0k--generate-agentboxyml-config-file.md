---
# agentbox-mp0k
title: Generate .agentbox.yml config file
status: completed
type: task
priority: low
created_at: 2026-04-02T10:34:46Z
updated_at: 2026-04-03T08:30:30Z
parent: agentbox-puuq
---

## Description
After generation, write a `.agentbox.yml` file to the project root recording the choices made:
...
This serves as documentation of what was generated and could enable a future `agentbox regenerate` command.

## Implementation Plan

### Approach

Add a `Config` struct and marshaling logic to `internal/config/config.go` (currently just a package doc comment), then call it from `cmd/init.go` after writing the `.devcontainer/` files. The config file is pure YAML with five top-level keys. Rather than adding a `gopkg.in/yaml.v3` dependency for this simple structure, use `encoding/json` internally with a hand-written YAML serializer. However, this is a false economy -- the YAML output needs to be human-readable, handle edge cases (empty lists, quoting timestamps), and a future `agentbox regenerate` will need to *read* the file back. Therefore, add `gopkg.in/yaml.v3` as a new dependency. This is the standard Go YAML library and a reasonable addition for a CLI tool that needs to read/write YAML config.
...
- `cmd/init.go` -- After writing `.devcontainer/` files, construct a `config.Config` and call `config.Write` to write `.agentbox.yml` to the project root
- `cmd/init_test.go` -- Add assertions that `.agentbox.yml` is generated with correct content
...
2. **Define `Config` struct in `internal/config/config.go`**
   ...
   - Define the struct:
     ...
     type Config struct {
         ...
         CcboxVersion string    `yaml:"agentbox_version"`
     }
     ```
   ...
   - `Version` is the schema version (always `1` for now), not the agentbox binary version. This field enables future schema migrations.
...
4. **Implement `Load` function**
   ...
   - This function enables a future `agentbox regenerate` command and makes round-trip testing possible now

5. **Implement `Filename` constant**
   - `const Filename = ".agentbox.yml"` -- single source of truth for the config filename, used by both `cmd/init.go` and future commands
...
7. **Integrate into `cmd/init.go`**
   - After the existing file-writing loop and chmod section, add:
     ...
     for i, id := range cfg.Stacks {
         agentboxCfg.Stacks[i] = string(id)
     }
     ```
   ...
   - Write the file:
     ```
     agentboxFile, err := os.Create(filepath.Join(dir, config.Filename))
     if err != nil {
         return fmt.Errorf("create %s: %w", config.Filename, err)
     }
     defer agentboxFile.Close()
     if err := config.Write(agentboxFile, agentboxCfg); err != nil {
         return fmt.Errorf("write %s: %w", config.Filename, err)
     }
     ```
   - The `.agentbox.yml` file is written to the project root (`dir`), not inside `.devcontainer/`. This is intentional: it is a project-level config file, like `.gitignore` or `.editorconfig`.
   - Add `"github.com/bjro/agentbox/internal/config"` and `"time"` to imports.

8. **Update `cmd/init_test.go`**
   - In `TestInitCommand_GeneratesDevcontainer`: add assertion that `.agentbox.yml` exists in the project root (not in `.devcontainer/`)
   - Add new test `TestInitCommand_CcboxYmlContent`: run init with `--stacks go,node --domains api.example.com`, then read `.agentbox.yml`, parse it with `config.Load`, and verify:
     - `Version` is 1
...
- **Integration tests in `cmd/init_test.go`**: Test that `agentbox init` produces a valid `.agentbox.yml` file with correct content.
