...
- **Single `RenderFirewall` function**: Takes a `GenerationConfig` and returns rendered content as a `FirewallFiles` struct (filename-to-bytes), deferring actual file writing to a later bean (the orchestrator that calls `agentbox init`). This keeps `render` pure and testable without filesystem I/O.
...
- No file I/O in this function -- pure transformation from config to rendered bytes. File writing is the responsibility of the orchestrator (`agentbox init` command).
