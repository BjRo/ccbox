...
## Description
End-to-end tests that run `agentbox init` against sample project directories and validate the full output:

1. Create temp dir with go.mod → run agentbox init --non-interactive → verify all 8 files exist with correct content
2. Create temp dir with package.json + go.mod → run agentbox init --non-interactive → verify multi-stack output
3. Create temp dir with existing .devcontainer/ → run agentbox init → verify it aborts with error message
4. Create temp dir with go.mod → run agentbox init --extra-domains "api.example.com" → verify domain appears in dynamic-domains.conf
5. Verify .agentbox.yml is written with correct stacks and options
...
Assertions:
- `.agentbox.yml` exists at `filepath.Join(dir, ".agentbox.yml")`.
