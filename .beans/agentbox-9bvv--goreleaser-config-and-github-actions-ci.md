...
**builds**: Single build entry for the `agentbox` binary.
- `main: .` (main.go is at repo root)
- `binary: agentbox`
- `env: [CGO_ENABLED=0]` for static binaries
- `goos: [linux, darwin]`
- `goarch: [amd64, arm64]`
- `ldflags: -s -w -X github.com/bjro/agentbox/cmd.version={{.Version}}`
...
- `license: MIT`
- `install`: Standard `bin.install "agentbox"` formula
- `test`: `system "#{bin}/agentbox", "--version"` to verify the formula works
...
3. **Smoke test after first tag**: The sibling bean `agentbox-xeg2` covers the end-to-end release verification.
