...
## Description
Implement the interactive wizard flow for `agentbox init` (default when no flags given):
...
4. **Boundary with sibling beans.** This bean does NOT implement `.agentbox.yml` writing (agentbox-mp0k) or the `--non-interactive` flag (agentbox-nvf1). The wizard outputs a `Choices` struct; downstream consumers decide what to do with it.
...
Package doc comment explaining that `wizard` implements the interactive prompt flow for `agentbox init`. It collects user choices (stack selection, extra domains) without performing any file I/O or template rendering.
...
3. **Manual smoke test**: Run `agentbox init` in a terminal, interact with the wizard, verify the generated .devcontainer/ looks correct. Run with `echo | agentbox init` to verify non-TTY fallback.
...
2. **Theme/styling.** The `huh` library supports themes (Charm, Dracula, Catppuccin, Base16, default). The plan uses the default theme, which looks good in most terminals. A custom theme could be added later to match agentbox branding, but this is cosmetic and out of scope for this bean.
...
>
> In the plan's Step 5 pseudocode, when the wizard runs (TTY + no `--stacks` flag), the `domains` CLI flag variable is never consulted. The wizard collects its own extra domains via the text input. If a user runs `agentbox init --domains api.example.com` without `--stacks`, the wizard fires and the `--domains` value is silently dropped.
>
> This is arguably correct behavior (the wizard supersedes flags), but it could surprise users. The sibling bean agentbox-nvf1 will add `--non-interactive` to address this fully, but until then there is a gap.
>
...
Finding 4 is a SUGGESTION that can be deferred to agentbox-nvf1 if preferred.
