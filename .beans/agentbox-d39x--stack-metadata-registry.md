...
Only Rust currently has a dynamic domain (`static.rust-lang.org` whose CDN IPs rotate). The field exists on every stack so the merging logic (agentbox-tv1t) and firewall package can handle both domain types uniformly without special-casing.
