...
1. **Create the templates directory and template file**
   ...
   ```json
   {
     ...
     "mounts": [
       "source=agentbox-bash-history,target=/home/node/.bash_history_volume,type=volume",
       "source=agentbox-claude-config,target=/home/node/.claude,type=volume",
   ...
   - The `mounts` array uses named Docker volumes (`agentbox-bash-history`, `agentbox-claude-config`) for persistent state that should survive container rebuilds (shell history, Claude configuration). The `agentbox-` prefix namespaces them to avoid collisions with other devcontainers.
   ...
   - `postStartCommand` chains two scripts: `sync-claude-settings.sh` (managed by sibling bean agentbox-v1zh) runs as the `node` user, then `init-firewall.sh` (managed by sibling bean agentbox-dttd) runs with `sudo` since iptables requires root.
...
3. **Write tests**
   ...
   - **`TestDevContainer_MountsContent`** -- Spot-checks the `mounts` array entries for expected substrings: `agentbox-bash-history`, `agentbox-claude-config`, `.config/gh`, `.gitconfig`. Verifies bind mounts use `${localEnv:HOME}`.
