...
# Feature flags and experimentation for Claude Code
for ip in $(dig +short 'statsig.com'); do
    if [[ "${ip}" =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
        ipset add allowed_ips "${ip}" -exist
    fi
done
